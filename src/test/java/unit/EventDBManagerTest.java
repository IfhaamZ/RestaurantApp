package unit;

import org.junit.jupiter.api.*;
import java.sql.Connection;
import java.sql.SQLException;
import java.time.LocalDate;
import java.util.List;

import static org.junit.jupiter.api.Assertions.*;

import model.Event;
import DAO.DBConnector;
import DAO.DBManager;

@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class EventDBManagerTest {

    private Connection conn;
    private DBManager dbManager;

    @BeforeAll
    public void setup() throws ClassNotFoundException, SQLException {
        conn = DBConnector.getConnection();
        dbManager = new DBManager(); // Adjust based on how DBManager is initialized
    }

    @AfterAll
    public void tearDown() throws SQLException {
        if (conn != null) {
            conn.close();
        }
    }

    @BeforeEach
    public void beforeEachTest() throws SQLException {
        conn.createStatement().execute("DELETE FROM Event"); // Clean the event table before each test
    }

    @Test
    public void testConnection() {
        assertNotNull(conn); // Ensure the connection is not null
    }

    @Test
    public void testInsertEvent() throws SQLException {
        Event event = new Event(0, "Music Night", "Enjoy live music", LocalDate.now(), "Music");
        boolean result = dbManager.createEvent(event);
        assertTrue(result);

        List<Event> eventList = dbManager.fetchAllEvents();
        assertEquals(1, eventList.size());
        Event insertedEvent = eventList.get(0);
        assertEquals("Music Night", insertedEvent.getName());
        assertEquals("Enjoy live music", insertedEvent.getDescription());
    }

    @Test
    public void testFetchAllEvents() throws SQLException {
        Event event1 = new Event(0, "Music Night", "Enjoy live music", LocalDate.now(), "Music");
        Event event2 = new Event(0, "Art Exhibition", "View various art pieces", LocalDate.now().plusDays(2), "Art");
        dbManager.createEvent(event1);
        dbManager.createEvent(event2);

        List<Event> eventList = dbManager.fetchAllEvents();
        assertEquals(2, eventList.size());

        Event fetchedEvent = eventList.get(1);
        assertEquals("Art Exhibition", fetchedEvent.getName());
        assertEquals("View various art pieces", fetchedEvent.getDescription());
    }

    @Test
    public void testUpdateEvent() throws SQLException {
        // Step 1: Create a new event
        Event event = new Event(0, "Music Night", "Enjoy live music", LocalDate.now(), "Music");
        dbManager.createEvent(event);

        // Step 2: Fetch the event again to ensure the ID is set
        List<Event> eventListBeforeUpdate = dbManager.fetchAllEvents();
        assertEquals(1, eventListBeforeUpdate.size());
        Event createdEvent = eventListBeforeUpdate.get(0);

        // Step 3: Create an updated event object using the correct ID
        Event updatedEvent = new Event(createdEvent.getId(), "Updated Music Night", "Updated Description",
                LocalDate.now().plusDays(1), "Music");

        // Step 4: Update the event
        boolean result = dbManager.updateEvent(updatedEvent);
        assertTrue(result);

        // Step 5: Fetch all events again and verify the update
        List<Event> eventListAfterUpdate = dbManager.fetchAllEvents();
        assertEquals(1, eventListAfterUpdate.size());
        Event fetchedEvent = eventListAfterUpdate.get(0);
        assertEquals("Updated Music Night", fetchedEvent.getName());
        assertEquals("Updated Description", fetchedEvent.getDescription());
        assertEquals(LocalDate.now().plusDays(1), fetchedEvent.getDate()); // Verify the date update
    }

    @Test
    public void testDeleteEvent() throws SQLException {
        // Step 1: Create a new event and insert it into the database
        Event event = new Event(0, "Music Night", "Enjoy live music", LocalDate.now(), "Music");
        dbManager.createEvent(event);

        // Step 2: Fetch the event again to ensure the ID is set
        List<Event> eventListBeforeDelete = dbManager.fetchAllEvents();
        assertEquals(1, eventListBeforeDelete.size());
        Event createdEvent = eventListBeforeDelete.get(0);

        // Step 3: Delete the event using its ID
        boolean result = dbManager.deleteEvent(createdEvent.getId());
        assertTrue(result); // Verify the delete operation was successful

        // Step 4: Verify that the event no longer exists in the database
        List<Event> eventList = dbManager.fetchAllEvents();
        boolean eventExists = eventList.stream().anyMatch(e -> e.getId() == createdEvent.getId());
        assertFalse(eventExists); // Check that the event with this ID no longer exists
    }

}
