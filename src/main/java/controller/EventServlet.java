...
public class EventServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        try {
            dbManager = new DBManager(); // Assuming DBManager is correctly set up for events
        } catch (Exception e) {
        ...
        try {
            switch (action) {
                ...
                case "/staffeventsearch":
                    searchEventsForStaff(request, response); // Add search event handling for staff
                    break;
                case "/staffeventlist":
    ...
    private void listEventsForCustomers(HttpServletRequest request, HttpServletResponse response)
    ...
    private void listEventsForStaff(HttpServletRequest request, HttpServletResponse response)
            throws SQLException, IOException, ServletException {
        List<Event> listEvents = dbManager.fetchAllEvents();
        request.setAttribute("events", listEvents);
        RequestDispatcher dispatcher = request.getRequestDispatcher("EventCalendarStaff.jsp");
        dispatcher.forward(request, response);
    }
    ...
    private void searchEvents(HttpServletRequest request, HttpServletResponse response)
        ...
        RequestDispatcher dispatcher = request.getRequestDispatcher("SearchEvent.jsp");
        dispatcher.forward(request, response);
    }

    // Search events for staff
    private void searchEventsForStaff(HttpServletRequest request, HttpServletResponse response)
            throws SQLException, IOException, ServletException {
        String type = request.getParameter("type");
        String date = request.getParameter("date");
        ...
        List<Event> listEvents = dbManager.searchEvents(type, date);
        request.setAttribute("ListOfEvents", listEvents);
        RequestDispatcher dispatcher = request.getRequestDispatcher("SearchEventStaff.jsp");
        dispatcher.forward(request, response);
    }
    ...
    private void insertEvent(HttpServletRequest request, HttpServletResponse response)
        ...
        Event newEvent = new Event(0, name, description, date, type); // ID 0 for new events
